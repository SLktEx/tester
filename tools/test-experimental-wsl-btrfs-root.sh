...
override_opts=$(
  WSL_BTRFS_MOUNT_OPTS='subvol=@,compress=zstd:3,noatime' bash -c 'source "$1"; printf "%s" "$MOUNT_OPTS"' _ "$SCRIPT"
)
assert_eq "$override_opts" 'subvol=@,compress=zstd:3,noatime'
printf 'ok 3 - mount option override is assembled from configuration\n'

verify_busybox_runtime /usr/bin/busybox
printf 'ok 4 - BusyBox required applets/options\n'
...
printf 'ok 5 - missing BusyBox is rejected\n'
...
set -e
assert_eq "$rc" 10
printf 'ok 6 - installed state is idempotent\n'
...
printf 'ok 7 - failed-install cleanup removes only created artifacts\n'
...
printf 'ok 8 - generated init wrapper preserves init arguments\n'
...
printf 'ok 9 - /mnt is carried as a propagated tree, not a drive list\n'
...
printf 'ok 10 - optional WSL runtime and kernel-module paths are feature-detected\n'

# Execute the generated optional-path helpers with mount operations mocked.
eval "$(sed -n '/^rbind_tree_if_exists()/,/^}/p' "$WRAPPER_PATH")"
eval "$(sed -n '/^carry_kernel_modules()/,/^}/p' "$WRAPPER_PATH")"

rbind_calls=''
rbind_tree() { rbind_calls=$1; }
touch "$TMP/present-runtime-path"
rbind_tree_if_exists "$TMP/present-runtime-path"
assert_eq "$rbind_calls" "$TMP/present-runtime-path"
rbind_calls=''
rbind_tree_if_exists "$TMP/missing-runtime-path"
assert_eq "$rbind_calls" ''

RUNTIME_MODULE_ROOT="$TMP/runtime-modules"
bb() {
  [[ "$1" == uname && "${2:-}" == -r ]] || return 1
  printf 'wsl-btrfs-test-kernel\n'
}
rbind_source_tree() { rbind_calls="$1 -> $2"; }
rbind_tree() { rbind_calls="normal:$1"; }
mkdir -p "$RUNTIME_MODULE_ROOT/lib/modules/wsl-btrfs-test-kernel"
rbind_calls=''
carry_kernel_modules
assert_eq "$rbind_calls" "$RUNTIME_MODULE_ROOT/lib/modules -> /usr/lib/modules"
rm -rf "$RUNTIME_MODULE_ROOT"
rbind_calls=''
carry_kernel_modules
if [[ -e /usr/lib/modules ]]; then
  assert_eq "$rbind_calls" 'normal:/usr/lib/modules'
else
  assert_eq "$rbind_calls" ''
fi
printf 'ok 11 - optional runtime paths handle present and absent cases\n'
...
printf 'ok 12 - active Btrfs root is a safe no-op\n'
...
printf 'ok 13 - --force refuses a still-active init wrapper\n'
...
printf 'ok 14 - --force cleans only a detected incomplete setup\n'
...
printf 'ok 15 - original init backup can be restored\n'
